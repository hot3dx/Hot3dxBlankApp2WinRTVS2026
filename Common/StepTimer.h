...
namespace DX
{
    ...
    public:
            ...
            m_targetElapsedTicks(TicksPerSecond / 60)
        {
            ...
            {
                throw winrt::hresult_error(E_FAIL, L"QueryPerformanceFrequency failed");
            }

            if (!QueryPerformanceCounter(&m_qpcLastTime))
            {
                throw winrt::hresult_error(E_FAIL, L"QueryPerformanceCounter failed");
            }
        ...
        void Tick(const TUpdate& update)
        {
            ...
            if (m_isFixedTimeStep)
            {
                ...
                if (std::llabs(static_cast<int64_t>(timeDelta - m_targetElapsedTicks)) < TicksPerSecond / 4000)
                {
